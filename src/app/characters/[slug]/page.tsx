...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    ...
    return { title: meta.alias, description: `${meta.alias} (${meta.name}) — ${meta.universe}` };
  } catch {
...
export default async function CharacterPage({ params }: PageProps) {
  ...
  const relatedEvents = getEventsForCharacter(meta.alias);
  ...
  return (
    <article className="grid grid-cols-1 gap-8 md:grid-cols-3">
      <aside className="comic-panel h-fit rounded-2xl border border-zinc-800 bg-zinc-900 p-4">
        <img src={meta.avatar} alt={meta.alias} className="mb-4 aspect-square w-full rounded-lg object-cover" />
        ...
        <div className="space-y-2 text-sm">
          ...
          <Fact label="Skapere" value={meta.creators && meta.creators.length > 0 ? meta.creators.join(", ") : undefined} />
          <Fact label="Også kjent som" value={meta.aliases && meta.aliases.length > 0 ? meta.aliases.join(", ") : undefined} />
          <Fact label="Lag" value={meta.teams.length > 0 ? meta.teams.join(", ") : "—"} />
        </div>
        <PowerStatsBars stats={meta.powerStats} />
        {relatedEvents.length > 0 && (
          <div className="mt-6 border-t border-zinc-800 pt-4">
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-zinc-500">Opptrådte i</h2>
            <ul className="mt-3 space-y-2 text-sm">
              {relatedEvents.map((event) => (
                <li key={event.slug}>
                  <Link href={`/events/${event.slug}`} className="text-zinc-200 hover:text-marvel-red">{event.title}</Link>
